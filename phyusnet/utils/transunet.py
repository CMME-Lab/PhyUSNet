"""
This file contains the implementation of the TransUNet.
Reference: https://github.com/mkara44/transunet_pytorch/tree/main
"""
...
class TransUNet(nn.Module):
    def __init__(
        ...
        img_dim:int  = 256,
        in_channels:int = 3,
        encoded_bottleneck_channels:int = 128,
        head_num:int = 4,
        mlp_dim:int = 512,
        block_num:int = 8,
        patch_dim:int = 16,
        ...
        self.decoder = Decoder(encoded_bottleneck_channels, class_num)
...
if __name__ == "__main__":
    ...
    transunet = TransUNet(
        img_dim=256,
        in_channels=3,
          encoded_bottleneck_channels=128,
        #   head_num=4,
        #   mlp_dim=512,
        #   block_num=8,
        #   patch_dim=16,
    ...
    transunet = transunet.to(torch.device("cuda:0"))

    print(str(sum(p.numel() for p in transunet.parameters())) + " parameters")
    print(transunet(torch.randn(1, 3, 256, 256).to(torch.device("cuda:0"))).shape)
