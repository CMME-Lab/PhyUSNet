...
try:
    ...
    from utils.eval import compute_hd95
    from utils.unet import UNet
    from utils.segformer import SegFormer
    from utils.swinunet import SwinUnet
    from utils.transunet import TransUNet
    from utils.swinunet_config import get_config
    from utils.swinunet import parser as swin_parser
...
class ClinicalUltrasoundDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # try:
        #     bbox = get_bounding_box(encoded_inputs["labels"],self.shift_bbox)
        # except Exception as e:
        #     print(e)
        #     print(f"Error Occured for sample {idx} : {x_img_path}")
        #     bbox = torch.tensor([-1,-1,-1,-1], dtype=torch.float32)
...
def get_bounding_box(mask, bbox_shift=400):
    """
    This function gets the bounding box of the mask
    Args:
        mask: torch.tensor, the mask
        bbox_shift: int, the shift of the bounding box
    Returns:
        bboxes: torch.tensor, the bounding box
    """
    mask = mask.cpu().numpy()
    try:
        ...
        x_min, x_max, y_min, y_max = -1, -1, 256, 256
...
def get_dataset(
    ...
    Args:
        benchmark_dataset_path: str, the path to the benchmark dataset
    ...
    dataset_path = os.path.join(benchmark_dataset_path, dataset_paths[dataset_key])
...
def visualize_batch_overlay(
    ...
    for idx, (image, gt_mask, pr_mask, x_path, x_label, x_dice, x_hd) in enumerate(
        ...
        data = {
            "input_image": image_np.tolist(),
            "ground_truth_mask": gt_mask_np.tolist(),
            "predicted_mask": pr_mask_np.tolist(),
        }
        # sample_idx = str(x_path[0]).split("\\")[-1].split(".")[0]
        if save:
            with open(os.path.join(save_dir, f"{sample_idx}.json"), "w") as f:
                json.dump(data, f)
...
def get_inference(
    ...
    for batch in tqdm(test_loader):
        ...
        if from_logits:
            ...
            pr_masks = logits # incase of unet and unet_pp
        
        if num_classes == 2:
            pr_masks = pr_masks.argmax(1)
            # pr_masks = pr_masks[:, 1:2, :, :]
...
class segformer_config(model_config):
    def __init__(self):
        ...
        self.remove_small_lesions = False
        self.threshold = 0.5
        self.num_classes = 2
...
def main(args):
    ...
    PLOTS_PATHS = os.path.join(RESULTS_DIR, args.task,BENCHMARK_DATASET, SELECTED_MODEL)
    POST_PROCESSING_FN = {
    ...
    if args.task == "clinical":
        CHECKPOINT_PATH = f"{args.checkpoint_folder_path}/task_specific_models/{SELECTED_MODEL}_clinical_{BENCHMARK_DATASET}"
        if (SELECTED_MODEL == "physformer") or (SELECTED_MODEL == "segformer"):
            config = SegformerConfig.from_pretrained(f"{CHECKPOINT_PATH}/config.json")
        else:
            config = None
    elif args.task == "physics":
        CHECKPOINT_PATH = f"{args.checkpoint_folder_path}/physics_models/physformer_{SELECTED_MODEL}"
    ...
    model_config.loaded_model_config = config
    model_config.loaded_checkpoint_path = CHECKPOINT_PATH
    model_config.device = device
    
    # Get the test loader
    _, _, _, _, _, test_dataset = get_dataset(
        dataset_key=BENCHMARK_DATASET,
        visualize=False,
    ...
    if model_config.loaded_checkpoint_path is None:
    ...
    else: # in the condition of already loaded the model from the checkpoint
        print("Model already loaded from the checkpoint")
    inference_config = {
        "SAVE_FIGURES": True,
        "ROOT_PATH": "",
        "MODEL_NAME": SELECTED_MODEL,
    ...
    df_results = get_inference(
        ...
        from_logits=inference_config["from_logits"],
        num_classes=inference_config["num_classes"],
    )
    print(f"Summary of the results:")
    print("Mean + /- Std")
    print(f"Dice: {df_results['dice'].mean():.4f} + /- {df_results['dice'].std():.4f}")
    # mean hd
    print(f"HD95: {df_results['hd95'].mean():.4f} + /- {df_results['hd95'].std():.4f}")
    # total count of test samples
    print(f"Total count of test samples: {len(df_results)}")
    # CSV Directory
    csv_dir_path = os.path.join(RESULTS_DIR,args.task, BENCHMARK_DATASET)
    os.makedirs(csv_dir_path, exist_ok=True)
    df_results.to_csv(f"{csv_dir_path}/{SELECTED_MODEL}_metrics.csv", index=False)
    # write the mean information in a logging text file for every command
    with open(os.path.join(RESULTS_DIR, "logging.txt"), "a") as f:
        f.write(f"Summary of the results:")
        f.write(f"Mean + /- Std")
        f.write(f"Dice: {df_results['dice'].mean():.4f} + /- {df_results['dice'].std():.4f}")
        f.write(f"HD95: {df_results['hd95'].mean():.4f} + /- {df_results['hd95'].std():.4f}")
        f.write(f"Total count of test samples: {len(df_results)}")
        # write the command used to run the script
        f.write(f"Command used to run the script: {args}")
...
if __name__ == "__main__":
    ...
    args.add_argument("--checkpoint_folder_path", type=str, default="./checkpoints")
    args.add_argument("--remove_small_lesions", type=bool, default=False)
    args.add_argument("--threshold", type=float, default=0.3)
    args.add_argument(
        "--benchmark_dataset_path",
        type=str,
        default="/home/user/haris/physics_medsam/benchmark_datasets/US30K/",
    )
    args.add_argument(
        "--results_dir",
        type=str,
    ...
    args.add_argument("--benchmark_dataset", type=str, default="BUSI")
    ...
    args.add_argument("--model_name", type=str, default="segformer", choices=["segformer", "unet", "swinunet", "transunet", "unet_pp","physformer"])
    
    args = args.parse_args()

    main(args)
    # # for model_name in ["unet_pp", "swinunet", "transunet","segformer"]:
    # for model_name in ["unet"]:
    #     for benchmark_dataset in ["BUSI", "UDIAT", "TN3K", "DDTI", "OvarianCyst"]:
    #         if (model_name == "unet") or (model_name == "unet_pp"):
    #             args.from_logits = False
    #         else:
    #             args.from_logits = True
            
    #         print(f"Running {model_name} on {benchmark_dataset}")
